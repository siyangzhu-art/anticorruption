//1.insepction//
use "C:\Users\siyang zhu\Downloads\inspection2013-2023.dta",clear
gen anti_shock = 0

replace anti_shock = 1 if monthstart != .
replace anti_shock = 1 if monthend != .

label var anti_shock "虚拟变量：该省该年是否受到政策/外部冲击"
label define shock_lab 0 "无冲击" 1 "受冲击"
label values anti_shock shock_lab


tab year anti_shock

save "inspection_Pyear.dta", replace
use "inspection_Pyear.dta", clear


keep if anti_shock == 1

sort province year
bysort province: gen round_rank = _n

gen ins_1 = year if round_rank == 1
gen ins_2 = year if round_rank == 2
gen ins_3 = year if round_rank == 3
gen ins_4 = year if round_rank == 4

collapse (min) ins_1 ins_2 ins_3 ins_4 , by(province)

gen gap_1_2 = ins_2 - ins_1
gen gap_2_3 = ins_3 - ins_2
gen gap_3_4 = ins_4 - ins_3
label var gap_1_2 "Gap between 1st and 2nd inspection (years)"



histogram ins_1, discrete width(1) frequency fcolor(ebblue%60) lcolor(ebblue) ///
    title("1st Inspection") xtitle("Year") ytitle("Frequency") ///
    xlabel(2013(1)2023, angle(45)) graphregion(color(white)) name(g1, replace)
histogram ins_2, discrete width(1) frequency fcolor(cranberry%60) lcolor(cranberry) ///
    title("2nd Inspection") xtitle("Year") ytitle("Frequency") ///
    xlabel(2013(1)2023, angle(45)) graphregion(color(white)) name(g2, replace)
histogram ins_3, discrete width(1) frequency fcolor(sea_green%60) lcolor(sea_green) ///
    title("3rd Inspection") xtitle("Year") ytitle("Frequency") ///
    xlabel(2013(1)2023, angle(45)) graphregion(color(white)) name(g3, replace)

graph combine g1 g2 g3, cols(2) rows(2) title("Inspection Years by Round (Separate Histograms)") ///
    graphregion(color(white)) name(ins_combined, replace)
graph export "ins_rounds_separate_combined.png", replace width(2000)

histogram gap_1_2, discrete width(1) frequency fcolor(ebblue%60) lcolor(ebblue) ///
    title("Gap: 1st → 2nd") xtitle("Years") ytitle("Frequency") ///
    xlabel(0(1)6) graphregion(color(white)) name(h1, replace)
histogram gap_2_3, discrete width(1) frequency fcolor(cranberry%60) lcolor(cranberry) ///
    title("Gap: 2nd → 3rd") xtitle("Years") ytitle("Frequency") ///
    xlabel(0(1)6) graphregion(color(white)) name(h2, replace)
graph combine h1 h2, cols(3) title("Gaps Between Consecutive Inspections (Separate)") ///
    graphregion(color(white)) name(gap_combined, replace)
graph export "gaps_separate_combined.png", replace width(2400)
use "inspection_Pyear.dta", clear
